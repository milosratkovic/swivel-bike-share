...
boolean Adafruit_FONA_LTE::HTTP_connect(const char *server) {
  ...
  // AT+SHCONF="URL","https://iot.swivel.bike"
...
boolean Adafruit_FONA::HTTP_ssl(boolean onoff) {
   mySerial->print("<<ENABLING SSL>>");
  sendCheckReply(F("AT+CSSLCFG=\"sslversion\",1,3"), ok_reply, 10000);
  return sendCheckReply(F("AT+SHSSL=1,\"\""), ok_reply,10000);
}
...
inline size_t Adafruit_FONA::write(const uint8_t *buffer, size_t length) {
  return mySerial->write(buffer, length);
}
